...
public class GameManager:MonoBehaviour{
	...
	public static bool autoplaying = false;
...
//		Application.LoadLevel("versesets");
	...
	public IEnumerator HandleCountTimeFinished() {
		if (scoreManager.isHighScore && scoreManager.WasVerseMastered()) {
			GetComponent<AudioSource>().PlayOneShot(victorySnd, 1.0f);
	...
	public void ShowSolution() {
		if (!CanShowSolution()) {
			...
			GetComponent<AudioSource>().PlayOneShot(sndFailure1,1.0f);
			return;
		}
		if (showingSolution) {
	...
	public void SetupWalls() {
		...
		topWall.offset = new Vector2(0f, mainCam.ScreenToWorldPoint(new Vector3(0f, (float)h ,0f)).y + 0.5f*thickness);	
		
		medWall.size = topWall.size;
		medWall.offset = new Vector2(0f, mainCam.ScreenToWorldPoint(new Vector3(0f, h*0.8f,0f)).y +thickness*0.1f);	
		
		bottomWall.size = topWall.size;
		bottomWall.offset = new Vector2(0f, mainCam.ScreenToWorldPoint(new Vector3(0f, 0f,0f)).y - thickness*0.5f);	
		
		leftWall.size = new Vector2(thickness, mainCam.ScreenToWorldPoint(new Vector3(0f, h*100.0f, 0f)).y);
		leftWall.offset = new Vector2(mainCam.ScreenToWorldPoint(new Vector3(0f, 0f,0f)).x - 0.5f*thickness, 0f);	
		
		rightWall.size = leftWall.size;
		rightWall.offset = new Vector2(mainCam.ScreenToWorldPoint(new Vector3((float)w, 0f, 0f)).x+0.5f*thickness, 0f);
		
		screenBounds = new Rect(leftWall.offset.x+0.5f*thickness,topWall.offset.y-0.5f*thickness,
		rightWall.offset.x-leftWall.offset.x-1.0f*thickness,
		topWall.offset.y-bottomWall.offset.y-1.0f*thickness);
	...
	public void HandleWordWrong() {
		...
		GetComponent<AudioSource>().PlayOneShot(sndFailure1, 0.5f);
	...
	public void HandleProgress() {
		...
		if ((!showingSolution && (scoreManager.timeLeft > 0)) ||
		    (p == 0)) {
			terrainManager.SetTargetProgress(p);
		} 
	}
	...
	public void ToggleAutoplay() {
		...
		if (autoplaying) {
			StartCoroutine(AutoplayNextWord());
		}
	}
	...
	public int HandleWordCorrect(WordLabel wordLabel) {
		...
		if (!autoplaying) {
			GetComponent<AudioSource>().PlayOneShot(snd, 0.25f);
		}
		...
		if (autoplaying || wasHinting) return 0;
	...
	public void SyncSetProgressLabel() {
		setProgressLabel.gameObject.SetActive(GetChallengeModeEnabled());
	...
	public void EnableWordColliders() {
		...
		foreach(WordLabel wordLabel in wordLabels) {
			wordLabel.GetComponent<Collider2D>().enabled = true;
		}
	}
	...
	public IEnumerator RecordPlay() {
		...
		if (verseset == null) {
			Debug.Log ("Skip record play because verse set not found");
			yield break;
		}
		...
		if (versesetId != null) {
			...
			StartCoroutine(ApiManager.GetInstance().CallApi("verseset/record_play", arguments, options));
		} else {
			Debug.Log ("Skip record play because verse set online id not found");
		}
	...
	public IEnumerator Start() {
		...
		if (needToRecordPlay) {
			Debug.Log ("start coroutine record play");
	...
	public void SetVerseReference(string reference,string version) {
		...
		string labelAndVersion = reference;

		if (version != null) {
			labelAndVersion += String.Format(" ({0})", version.ToLower());
		}
		
		introReferenceLabel.text = label;
		panelReferenceLabel.text = labelAndVersion;
...
//		Debug.Log ("result = " + result + " pattern = " + pattern);
	...
	public void UpdateGravityScale() {
		...
		foreach(WordLabel wordLabel in activeWordLabels) {
			wordLabel.GetComponent<Rigidbody2D>().gravityScale = gravity;
		}
	...
	public void scrambleWordLabels() {
		...
		int randomIndex = 0;
	  	...
	  	while (0 != currentIndex) {
	    	...
	    	randomIndex = (int)((currentIndex - g) + Mathf.Floor(UnityEngine.Random.Range(0.0f,1.0f) * g));
...
//		float maxX = screenBounds.x + screenBounds.width;
	...
	public IEnumerator SetupVerse() {
		...
		wordIndex = 0;
		currentWord = words[wordIndex];
		Debug.Log ("words = " + JSONUtils.ArrayToJSON(words));
	...
	public int ReleaseWords(int index,int numWords) {
        ...
        for(i=index;i<scrambledWordLabels.Count;i++) {
			...
			wordObject.GetComponent<Rigidbody2D>().isKinematic = false;
			activeWordLabels.Add(wordObject);
			//Debug.Log (i + ". " + wordObject.word);
			c += 1;	
			if (c == numWords) {
				break;
			}
	...
	public void HandleRanOutOfTime() {
		...
		if (GetChallengeModeEnabled()) {
			foreach(WordLabel wordLabel in wordLabels) {
				wordLabel.GetComponent<Collider2D>().enabled = false;
			}
	...
	public void ShowHintFromButton() {
		...
		GetComponent<AudioSource>().PlayOneShot(sndSuccess1, 0.5f);
	}

	public float GetAutoplayDelay() {
		switch(difficulty) {
	...
	public IEnumerator AutoplayNextWord() {
		yield return new WaitForSeconds(GetAutoplayDelay());
		if (!autoplaying) yield break;
		WordLabel wObject = CurrentAvailableWordLabel();
		if ((wObject != null) && gameStarted) {
			scoreManager.UpdateHealthBar(0);
			scoreManager.mistakes += 1;
			StartCoroutine(wObject.OnMouseDown());
		} 
	}

	public WordLabel CurrentAvailableWordLabel() {
		if ((wordIndex < 0) || (wordIndex >= wordLabels.Count)) return null;
	...
	public void Update() {
		...
		refreshButton.gameObject.SetActive(CanShowSolution() || (finished && !GetChallengeModeEnabled()));
		hintButton.gameObject.SetActive(!GetChallengeModeEnabled());
		shuffleButton.gameObject.SetActive(GetChallengeModeEnabled());

		if (VerseManager.shuffled) {
			shuffleButton.SetImageAlpha(1f);
		} else {
			shuffleButton.SetImageAlpha(0.5f);
		}

		if (autoplaying) {
			autoplayButton.SetImageAlpha(1f);
		} else {
			autoplayButton.SetImageAlpha(0.5f);
		}
	...
	public void ToggleShuffled() {
		bool shuffled = VerseManager.shuffled;
		VerseManager.SetShuffled(!shuffled);
		Cleanup();
		StartChallenge();
	}
